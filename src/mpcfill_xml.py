...
if __name__ == "__main__":
    ...
    parser.add_argument("--equal_brackets", action="store_true", help="If the orders should be broken up into two equal orders or the largest possible and next smallest. I.E if a 1008 card order should be 612,396 or 504,504")
    ...
    back_counts = len([path for path in draftable_cards if path.__contains__("_2.png")])
    total = len(draftable_cards)+len(token_cards)-back_counts
    
    orders = []
    if not args.equal_brackets:
        cards_left = total
    ...
    for idx, order in enumerate(orders):
        ...
        make_order(order_list, order, str(args.card_back.absolute()), args.cardstock, args.foil, f"order{order}_{idx}.xml")
